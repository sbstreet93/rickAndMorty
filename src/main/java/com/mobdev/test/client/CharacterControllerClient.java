package com.mobdev.test.client;

import com.mobdev.test.config.feign.CharacterControllerClientConfig;
import com.mobdev.test.client.model.CharacterServerDto;
import org.springframework.cloud.openfeign.FeignClient;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;

import static org.springframework.http.MediaType.APPLICATION_JSON_VALUE;

@FeignClient(value = "charactercontroller", url = "${service.rick.and.morty.character.url}",
        configuration = CharacterControllerClientConfig.class)
public interface CharacterControllerClient {
    @GetMapping(path = "/{id}", produces = APPLICATION_JSON_VALUE)
    CharacterServerDto getById(@PathVariable Integer id);
}